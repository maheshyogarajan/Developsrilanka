"""
Upgrade key organizations to the comprehensive 74-account chart of accounts.
"""
import sys
import os
...
def get_comprehensive_accounts():
    ...
    return [
        ...
        # COST OF GOODS SOLD (3 accounts) - Note: using 'expense' type for compatibility
...
def upgrade_key_organizations():
    """Upgrade key organizations to the comprehensive chart of accounts."""
    
    with app.app_context():
        try:
            # Key organizations to upgrade
            key_orgs = [
                'Personal Finances',
            ...
            print(f"🔄 Upgrading {len(key_orgs)} key organizations to 74-account structure...")
            
            for org_name in key_orgs:
                org = Organization.query.filter_by(name=org_name).first()
                
                if org:
                    new_count, updated_count = upgrade_organization_accounts(org.id, org_name)
                    print(f"  ✅ {org_name}: +{new_count} new, ~{updated_count} updated")
                    total_new += new_count
            ...
            print(f"\n🎉 KEY ORGANIZATIONS UPGRADE COMPLETE!")
...
if __name__ == "__main__":
    print("🚀 Starting key organizations chart of accounts upgrade...")
    upgrade_key_organizations()