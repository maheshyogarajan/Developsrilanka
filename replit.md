...
- Admin-only health endpoint: `GET /admin/celery_health` returns the live worker count and, for each worker, the name plus how many seconds ago it last checked in. Liveness is read from the `worker_heartbeat` Postgres table (the SQLAlchemy Celery broker doesn't support the fanout broadcast that `celery inspect`/`ping` rely on, so each worker upserts its `last_seen` timestamp every 15 s from a background thread in `worker.py`). The same table powers the `/scan` worker-availability check; rows older than 60 s are treated as offline. Use this endpoint to confirm the worker is alive in production after deploys.
